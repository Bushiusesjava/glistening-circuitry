...
const PROJECTS = [
  ...
  },
  {
    ...
    time: "1 hr",
    cost: "$8",
    blurb: "A lamp that switches on when it gets dark — built purely from analog parts, no microcontroller.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Add a 100 µF capacitor right at each module's power pins — the radio draws sharp current spikes.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Plug in headphones and A/B it against a phone's output — listen for noise floor.",
    ],
...
const LESSONS = [
  ...
  },
  {
    ...
    html: `
      <p><strong>Series</strong> means parts are chained one after another — current flows through each in turn. <strong>Parallel</strong> means parts sit side by side across the same two nodes — each gets the full supply voltage.</p>
      <table>
      ...
      <p>Resistors add up in series, so total resistance <em>grows</em>. In parallel the total is always <em>smaller</em> than the smallest branch — each new path lets more current through.</p>
  ...
  },
  {
    ...
    html: `
      ...
      <p>β (beta, current gain) is often 100–300. Drive the base hard enough and the transistor <strong>saturates</strong> — it switches fully on and the collector sits near ground, like a closed switch.</p>
  ...
  },
  {
    ...
    html: `
      <p>An operational amplifier ('op-amp') is a differential amplifier with huge gain: it amplifies the difference between its two inputs. Used <em>without</em> feedback it's a comparator — output slams high or low. Used <em>with</em> feedback it becomes a precise, predictable amplifier.</p>
      <p><strong>The golden rules</strong> (when feedback is applied):</p>
      <ul>
        <li>The inputs draw (almost) no current.</li>
        <li>Feedback makes the inputs try to equalize — 'virtual ground' on the inverting input.</li>
      </ul>
      ...
      <blockquote><p><strong>Practice:</strong> with R<sub>f</sub> = 100 kΩ and R<sub>g</sub> = 10 kΩ, non-inverting gain is 1 + 100k/10k = <strong>11×</strong>. A 0.2 V input becomes 2.2 V. Just remember the rails are the ceiling — you can't amplify beyond the supply.</p></blockquote>
    `,
  },
  {
    id: "analog-digital",
    ...
    html: `
      ...
      <p><strong>PWM — pulse width modulation.</strong> To fake a variable voltage on a digital pin, the pin flips fast between 0 and 5 V and you vary the duty cycle (percent high). Average voltage = duty × 5 V. This dims LEDs and sets motor speed — <code>analogWrite()</code>.</p>
      <blockquote><p>Key wiring habit: <strong>pull-up / pull-down resistors.</strong> A bare switch leaves a pin floating — noise decides its state. Pull the pin to 5 V with a 10 kΩ (pull-up) or to ground (pull-down), and the switch then does the opposite cleanly. Many chips also have internal pull-ups you can enable in software.</p></blockquote>
      ...
      <p>And the classic beginner trap: <strong>when the smoke escapes, it's usually the current</strong>. A microcontroller pin delivers at most ~40 mA — always switch heavier loads through a transistor or driver, never straight off a pin.</p>
    `,
  },
  {
    id: "schematics",
    ...
    html: `
      ...
      <blockquote><p><strong>Reading a datasheet is a skill, not knowledge.</strong> Don't read it end to end. Ask a question — 'can this switch 3 A?' — then jump to the exact table that answers it.</p></blockquote>
    `,
  },
];
